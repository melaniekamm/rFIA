  ...
  if (byPlot & !treeList){
    ...
    t <- data %>%
      ...
      dplyr::summarize(TPA = sum(TPA_UNADJ * tDI, na.rm = TRUE),
                       ...
                       CPA = sum(!!CUSTVAR * TPA_UNADJ * tDI, na.rm = TRUE)) %>%
      as.data.frame() %>%
...
tpa <- function(db,
  ...
  out <- lapply(X = iter, FUN = tpaStarter, db,
                ...
                nCores, remote, mr, custVar_quo = custVar_quo)
  ...
  if (!byPlot & !treeList){
    ...
    tEst <- tEst %>%
      ...
      dplyr::mutate(TREE_TOTAL = tPlot_mean,
                    ...
                    CPA_VAR = ratioVar(cPlot_mean, fa_mean, cPlot_var, fa_var, cPlot_cv),
                    # Sampling Errors
                    TREE_TOTAL_SE = sqrt(tPlot_var) / tPlot_mean * 100,
                    BA_TOTAL_SE = sqrt(bPlot_var) / bPlot_mean * 100,
                    CUST_TOTAL_SE = sqrt(cPlot_var) / cPlot_mean * 100,
                    AREA_TOTAL_SE = sqrt(fa_var) / fa_mean * 100,
                    TPA_SE = sqrt(TPA_VAR) / TPA * 100,
                    BAA_SE = sqrt(BAA_VAR) / BAA * 100,
                    CPA_SE = sqrt(CPA_VAR) / CPA * 100,
                    # Plot counts
                    nPlots_TREE = nPlots.x,
                    nPlots_AREA = nPlots.y,
                    N = P2PNTCNT_EU) %>%
      dplyr::select(!!!grpSyms, TPA, BAA, CPA, TREE_TOTAL, BA_TOTAL, CUST_TOTAL, AREA_TOTAL,
                    TPA_VAR, BAA_VAR, CPA_VAR, TREE_TOTAL_VAR, BA_TOTAL_VAR, CUST_TOTAL_VAR, AREA_TOTAL_VAR,
                    TPA_SE, BAA_SE, CPA_SE, TREE_TOTAL_SE, BA_TOTAL_SE, CUST_TOTAL_SE, AREA_TOTAL_SE,
  ...
  if(is.null(custVar_quo)) {
    tEst <- dplyr::select(tEst, -dplyr::starts_with('CUST'), -dplyr::starts_with('CPA'))
  }
